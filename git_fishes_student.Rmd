...
[My GitHub Repository](https://github.com/YouzhiHuang/Version-Control-Git-and-GitHub/commits/main)
...
This is an online platform and, depending on the network, it is not always accessible. For example in China it is not accessible.
```
...
path1 <- "./output/data/dt"

write_csv(dt, file = path1)
...
## The red and green highlights in the online presentation are the deleted and added lines of code respectively
```
...
```{r, collabFig,fig.cap="A figure [@ref(fig:collabFig)]", echo=TRUE, eval=TRUE}
...
```{r, table1, echo=TRUE, eval=TRUE, tab.cap = "t statistical tests to determine for each species"}
...
> **Question 8**: Pick one of your favorite species and write about the results to a reader. Write in the Rmarkdown file below what 1) the means and mean differences between control and acidification treatment is along with 
2) the 95% confidence intervals of the difference

> 1) In humbug species, the mean values of standard length in fish for control and acidified treatment groups were 31.77 mm and 31.69 mm, respectively, with a mean difference of 0.09. The mean values of activity in fish were 34.92 (s / min) and 40.28 (s / min), respectively, with a mean difference of 5.37 (s / min). In humbug species, the mean values of standard length in fish for control and acidified treatment groups were 31.77 mm and 31.69 mm, respectively, with a mean difference of 0.09 mm.
  2) The 95% confidence intervals for the differences ranged from -4.04 to 3.86 for the standard length of the fish and 0.83 to 9.90 for the activity of the fish.
...
Mean activity for `r "humbug"` in the control group was `r actresult$estimate2[which(actresult$species=="humbug")]` (s / min) compared to the OA treatment group, which was `r actresult$estimate1[which(actresult$species=="humbug")]` (s / min). The difference between control and OA treatment means was `r actresult$estimate[which(actresult$species=="humbug")]` (s / min) (95% CI: `r actresult$conf.low[which(actresult$species=="humbug")]` to `r actresult$conf.high[which(actresult$species=="humbug")]`). 
...
**Write your answer here**
The null hypothesis being tested is that the mean activity of humbug species is not statistically significantly different in the control and OA treatment groups. Based on the results of the t-test: t-statistic of `r actresult$statistic[which(actresult$species=="humbug")]`, df of `r actresult$df[which(actresult$species=="humbug")]` and p-value of `r actresult$p[which(actresult$species=="humbug")]` less than α = 0.05, the null hypothesis was rejected and the alternative hypothesis that the mean activity of the humbug species was statistically significantly different between the control and OA treatment groups was accepted (significant level α = 0.05).
...
The results of the ranked combination test showed p-values greater than 0.05 in each species(ambon: `r permutation_result$P_value[which(permutation_result$species=="ambon")]` lemon: `r permutation_result$P_value[which(permutation_result$species=="lemon")]` chromis: `r permutation_result$P_value[which(permutation_result$species=="chromis")]` acantho: `r permutation_result$P_value[which(permutation_result$species=="acantho")]` humbug: `r permutation_result$P_value[which(permutation_result$species=="humbug")]` whitedams: `r permutation_result$P_value[which(permutation_result$species=="whitedams")]`), indicating that the mean activity of each species was not statistically significantly different between the control and OA treatment groups (significance level α= 0.05).
